...
# -----------------------------
# Эти признаки помогают сети уловить соотношения "цена/железо",
# а не только абсолютные значения.
eps = 1e-6
...
# -----------------------------
# 2.2) NLP/Regex фичи из name/description
# -----------------------------
def safe_lower(s):
    if pd.isna(s):
        return ""
...
def extract_cpu_tier(text):
    # унифицированный CPU tier для Intel/Ryzen
    ...
    apple = re.search(r"\ba\d{2}\s*bionic\b", text)
    if apple:
        return 8.0
    return 0.0

def extract_gpu_tier(text):
    # грубая шкала gpu уровня
...
# взаимодействия текста и числовых признаков
...
# Для устойчивости регрессии по ценам
y = np.log1p(y_raw).astype(np.float32)

# Стратификация: одновременно по типу устройства и диапазону цены.
price_bins = pd.qcut(
    y_raw, q=6, labels=False, duplicates="drop"
).astype(str)
stratify_key = df["device_type_label"].astype(str) + "_" + price_bins
# Для небольших датасетов некоторые сочетания device+bin могут встретиться 1 раз,
# что ломает stratified split. Редкие группы откатываем к device_type.
...
# Если после отката все еще есть группы с 1 элементом, сливаем их в общий класс.
...
if use_stratify:
    ...
    print("Предупреждение: stratify отключен из-за слишком редких групп.")
    df_train, df_val = train_test_split(
...
# Leak-safe target encoding: считаем только по train
def add_target_encoding(train_df, val_df, col, target, min_count=10, alpha=20.0):
    ...
    # редкие категории отправляем в global_mean
    ...
    return (
        train_enc.astype(np.float32),
        val_enc.astype(np.float32),
        {str(k): float(v) for k, v in smooth.to_dict().items()},
        float(global_mean),
    )

# CPU/GPU family для target encoding
def cpu_family(text):
...
te_artifacts = {}
for col in enc_cols:
    tr_enc, va_enc, te_map, te_global = add_target_encoding(
        df_train, df_val, col, target_col, min_count=10, alpha=25.0
    )
    df_train[f"te_{col}"] = tr_enc
    df_val[f"te_{col}"] = va_enc
    te_artifacts[col] = {"map": te_map, "global_mean": te_global}

# финальные фичи: только numeric, без исходных строковых текстов
exclude_cols = {
...
# Балансировка батчей по стратификационным группам
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for xb, yb in val_loader:
                ...
                all_preds_log.extend(preds.detach().cpu().numpy())
                all_true_log.extend(yb.detach().cpu().numpy())
...
# Ансамбль (медиана по предсказаниям лучших эпох каждой модели)
...
print("Валидационные прогнозы ансамбля сохранены в artifacts/*.npy")
...
palette = {
    "train": "#7dd3fc",   # мягкий голубой
    "val": "#f9a8d4",     # мягкий розовый
    "mae": "#86efac",     # мягкий зеленый
    "mape": "#fcd34d",    # мягкий янтарный
}
...
print("\nИтог на валидации (лучшая одиночная модель):")
print(f"seed = {best_single['seed']}")
print(f"Лучшая эпоха = {best_single['best_epoch']}")
print(f"MAE = {best_single['best_metrics'].get('mae', float('nan')):.2f} руб")
print(f"MAPE = {best_single['best_metrics'].get('mape', float('nan')):.2f}%")
print(f"MAPE@5k = {best_single['best_metrics'].get('mape_floor', float('nan')):.2f}%")
print(f"SMAPE = {best_single['best_metrics'].get('smape', float('nan')):.2f}%")

print("\nИтог на валидации (ансамбль):")
print(f"MAE = {ensemble_metrics['mae']:.2f} руб")
print(f"MAPE = {ensemble_metrics['mape']:.2f}%")
print(f"MAPE@5k = {ensemble_metrics['mape_floor']:.2f}%")
print(f"SMAPE = {ensemble_metrics['smape']:.2f}%")
...
# Берём 10 случайных индексов из валидации (для воспроизводимости можно задать seed)
rng = np.random.RandomState(42)
sample_idx = rng.choice(len(y_raw_val), size=10, replace=False)

for i in sample_idx:
    real_price = y_raw_val[i]
    pred_price = ensemble_preds_rub[i]
    # Пытаемся достать название устройства (если колонка 'name' есть в df_val)
    try:
